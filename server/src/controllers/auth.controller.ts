    ...
    public signup = (req: Request, res: Response, next: NextFunction): Promise<Response> =>
        passport.authenticate("register", (err, user, info) => {
            if (!signUpPassportErrors(res, err, info)) {
                return apiResponse(
                    res,
                    successResponse({ message: `User ${user.username} has been created successfully` }),
                    201
                );
            }
        })(req, res, next);
                ...
                if (user.user_id === req.body.user_id) {
                    try {
                        ...
                        if (userUpdated[0] === 1) {
                            ...
                            return apiResponse(
                                res,
                                successResponse({ message: `Username "${username}" was updated successfully.` }),
                                200
                            );
                        }
                        ...
                        return apiResponse(
                            res,
                            failedResponse({ message: `Username "${username}" doesn't exist` }),
                            404
                        );
                    } catch (error) {
                        logger.error("[Error] Update User information: ", { meta: { ...(error as object) } });
    ...
    public forgotPassword = async (req: Request, res: Response, next: NextFunction) => {
        try {
            ...
            if (userFound) {
                ...
                const tokenResetPassword = await this.authService.createTokenForLinkToResetPassword(
                    userFound.user_id,
                    token
                );
                if (tokenResetPassword[0] === 1) {
                    ...
                    return apiResponse(
                        res,
                        failedResponse("Error trying to create the token for reseting password."),
                        400
                    );
                }
            } else {
                ...
                return apiResponse(
                    res,
                    failedResponse("Email is not recognized. Please try again or register for a new account."),
                    404
                );
            }
        } catch (error) {
            logger.error("Error trying to create the token for reseting password.", { meta: { ...(error as object) } });
            return apiResponse(res, failedResponse(error), 400);
        }
    };
    ...
    public resetPassword = async (req: Request, res: Response, next: NextFunction) => {
        try {
            ...
            if (userFound) {
                ...
                return apiResponse(
                    res,
                    successResponse({ username: userFound.username, message: "Password reset link a-ok" }),
                    200
                );
            }
        ...
        try {
            ...
            if (userFound) {
                ...
                if (passwordUpdated[0] === 1) {
                    ...
                    return apiResponse(
                        res,
                        successResponse("Password has been successfully updated, please try logging in again."),
                        200
                    );
                }
                logger.error("Error trying to create the token for reseting password.");
            ...
            logger.error("Error trying to update the password.", { meta: { ...(error as object) } });
            return apiResponse(res, failedResponse(error), 400);
        }
    };
}
