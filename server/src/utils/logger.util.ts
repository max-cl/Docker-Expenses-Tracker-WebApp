...
export const myFormat = printf((info) => {
    return `[${info.timestamp}] [${info.level}] [38;5;13m[1m=>[22m[39m ${info.message} ${info.meta !== undefined ? info.meta : ""}`;
});

export const logger = winston.createLogger({
    levels: custom.levels,
    format: combine(label({ label: "order-api errors" }), timestamp(), colorize({ colors: custom.colors }), json(), myFormat),
